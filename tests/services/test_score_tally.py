from app.models import ScoreVote, Meeting, Motion, Option, User, Voter
from app.services.voting import tally_score_votes

def test_score_tally_one_winner(db_session):
    admin = User(
        username="svc_admin",
    ...
    result = tally_score_votes(motion)
...
def test_score_tally_tiebreak_one_winner(db_session):
    admin = User(
        username="svc_admin",
        email="svc_admin@example.com",
        password_hash="hashed-password",
    ...
    assert result["deadlock"] is False
    assert len(result["winners"]) == 1
    assert result["winners"][0].id == option_b.id

def test_score_tally_deadlock_when_point_level_distribution_matches(db_session):
    admin = User(
        username="svc_admin",
        email="svc_admin@example.com",
        password_hash="hashed-password",
